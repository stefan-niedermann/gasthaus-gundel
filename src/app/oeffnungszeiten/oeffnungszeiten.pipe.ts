...
export class OeffnungszeitenPipe implements PipeTransform {
  ...
  transform(value: Oeffnungszeit[], args?: any): any {
    ...
    value.map((oeffnungszeit: Oeffnungszeit) => {
      oeffnungszeitenStr += `${this.decimal.transform(oeffnungszeit.von.h, '2.0')}:${this.decimal.transform(oeffnungszeit.von.m, '2.0')} Uhr`;
      if (oeffnungszeit.bis) {
        oeffnungszeitenStr += ` - ${this.decimal.transform(oeffnungszeit.bis.h, '2.0')}:${this.decimal.transform(oeffnungszeit.bis.m, '2.0')} Uhr`;
      } else {
